...
/* 트리의 부모 노드 찾기
* - 무방향 그래프로 주어진 트리의 간선 정보를 이용하여 각 노드의 부모 노드 찾기
* - DFS, BFS를 이용하여 탐색
* - 인접 리스트(각 노드에 연결된 노드(== 간선 정보) 정보 저장)
* */
...
public class F_FindTreeParent {
  ...
  public static String solution(String input) throws IOException{
    ...
    // 입력 첫 번째 줄 == 노드 개수
    N = Integer.parseInt(br.readLine()); //한줄읽어온거 int로 파싱해서

    // 배열 할당, 인덱스 번호 == 노드 번호
    parent = new int[N+1]; // 1번부터 시작
    visit = new boolean[N+1];
    list = new ArrayList[N+1]; //ArrayList 참조변수 배열을 만드는 것임. 그래서 ArrayList<>[N+1]로 ㄴㄴ
  ...
  private static void dfs(int parentNode){
    ...
    // list[parentNode]에는 parentNode와 연결된 모든 노드 정보가 입력되어있음
   ...
   * - 루트부터 레벨순서로 탐색하며 부모-자식 관계를 저장
   *
   * @param root (시작 노드, 1번)
   */
  private static void bfs(int root){ //현재 child노드의 부모 노드 찾기가 목표
